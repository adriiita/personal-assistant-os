...
- **Session Logs** (`.claude/context/session-logs/`) — Saved summaries of past work sessions. Used by `/resume` to reconstruct context.
...
- `/resume` — Start a session by loading recent context and pending tasks
- `/compress` — Save current session before ending (creates searchable session log)
- `/preserve` — Save permanent learnings to memory (with auto-archive)
- `/daily-review` — Start your morning, evening, or weekly routine
